...
| 13 | `inventory_items` | Master | Nguyên liệu tồn kho | Should |
| 14 | `inventory_transactions` | Transaction | Nhập/xuất/kiểm kê nguyên liệu | Should |
| 15 | `audit_logs` | Transaction | Nhật ký hoạt động (INSERT-only) | Must |
...
#### `inventory_items` — Nguyên liệu (Should Have)

| Cột | Kiểu | Ghi chú |
|-----|------|---------|
| id | INT | Khóa chính |
| name | VARCHAR(200) | Tên nguyên liệu |
| unit | VARCHAR(20) | Đơn vị tính (kg, lít, cái...) |
| quantity | DECIMAL(12,3) | Tồn kho hiện tại |
| min_quantity | DECIMAL(12,3) | Tồn tối thiểu (cảnh báo) |
| status | ENUM(VARCHAR) | CON_HANG, SAP_HET, HET_HANG, NGUNG_SU_DUNG |
| created_at | DATETIME(3) | |
| updated_at | DATETIME(3) | |
| deleted_at | DATETIME(3) | Soft delete |
...
| type | ENUM(VARCHAR) | NHAP_KHO, XUAT_KHO, KIEM_KE, DIEU_CHINH |
| quantity | DECIMAL(12,3) | Số lượng (+ nhập, − xuất) |
| note | TEXT | Ghi chú (nullable) |
| created_by | INT | FK → users.id (KHO) |
| created_at | DATETIME(3) | |
...
| `inventory_items` | `status` | `CON_HANG`, `SAP_HET`, `HET_HANG`, `NGUNG_SU_DUNG` |
...
| `inventory_transactions` | `type` | `NHAP_KHO`, `XUAT_KHO`, `KIEM_KE`, `DIEU_CHINH` |
...
- Các bảng master (users, staff, tables, menu_categories, menu_items, reservations, inventory_items) có `deleted_at`.
...
| Q-DB-05 | inventory_items có cần link với menu_items không? | Chưa (Should Have) | File 07, Q-INV-02 |
